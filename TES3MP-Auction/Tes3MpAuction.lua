...
local SGuI = 44332220
local IGuI = 44332225
local PostItemLimit = 100 -- (Not Working) max amount of items posted allowed
local BaseItemName = {}
local Auction = {}
local config = {}
local SellValue = ""
...
Auction.Option = function(pid)
      if Players[pid] ~= nil and Players[pid]:IsLoggedIn() then
         -- Auction.Bot(pid) (Not Functional)
...
Auction.Sell = function(pid)
      if Players[pid] ~= nil and Players[pid]:IsLoggedIn() then
         if Auction.CheckLimit(pid) == true then
...
customEventHooks.registerHandler("OnGUIAction", function(eventStatus, pid, idGui, data)
    ...
    if isValid ~= false then
        ...
        if idGui == IGuI then
           if data ~= nil then
              SellValue = tostring(data)
              return Auction.SaveItem(pid)
           end
        end
        if idGui == GuI then
             if data ~= nil then
                if tonumber(data) <= PostItemLimit then
                   Iname = tonumber(data)
...
Auction.BuyItem = function(pid)
      if Players[pid] ~= nil and Players[pid]:IsLoggedIn() then
         ...
         local Iname = Iname + 1
               for i=1,#Buying do
                   if In == Iname then
                      --- local Msg = Buying[In].Player .. " " .. Buying[In].Name .. " " .. Buying[In].Price
...
Auction.Bot = function(pid)
     if Players[pid] ~= nil and Players[pid]:IsLoggedIn() then
        local itemBot
        local ItemLoad
              ItemLoad = jsonInterface.load("Auction/StoredItems.json")
                  if ItemLoad ~= nil then

                  else
                     ItemBot = jsonInterface.load("Auction/BotItems.json")
                     jsonInterface.save("Auction/StoredItems.json", ItemBot)
                  end          
     end
end
